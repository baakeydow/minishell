...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/04/26 02:40:07 by bndao             #+#    #+#             */
/*   Updated: 2016/04/26 02:40:22 by bndao            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int         setenv_tpl(char *key, char *value, int overwrite, t_data *d)
{
    if (key_exist(d->env, key))
    {
        while (d->e)
        {
            ...
            {
                d->e->value = val_new(value);
                ft_putstr(d->e->value->entry);
            }
            d->e = d->e->next;
        }
        while (d->e->prev)
...
{
    if ((!av[0] || !av[1] || !av[2] || !is_in(av[1], '=') ||
            ft_strcmp(ft_strtrim(av[0]), "setenv") != 0) ||
